...
\hebrewversion{גרסה 5.0 - הדגמה מלאה}
...
\hline
% \hebcell for Hebrew cells
\hebcell{תא עברי} &
% \encell for English cells
\encell{English cell} &
% \mixedcell (backward compatible)
...
\hebrewfigure[h]{
    \fbox{\parbox{10cm}{
        \centering
        \vspace{3cm}
        איור עם פקודת \texttt{hebrewfigure}\\
        \en{Figure using hebrewfigure command}\\
        \vspace{3cm}
    }}
...
\fbox{\parbox{10cm}{
    \centering
    \vspace{3cm}
    איור עם סביבת \texttt{figure}\\
    \en{Figure using figure environment}\\
    \vspace{3cm}
}}
...
\begin{pythonbox}[דוגמת קוד צף: \en{Floating Code Example}]
# Demonstrate Python code with syntax highlighting
def fibonacci(n):
...
\begin{pythonbox*}[קוד ארוך לא צף: \en{Long Non-Floating Code}]
# Non-floating code block for longer listings
class DataProcessor:
...
\begin{rtltabular}{|p{4cm}|p{3cm}|p{3cm}|}
\hline
...
\begin{pythonbox}[מימוש \en{Attention} עם \hebmath{קשב}]
import torch
...
\hebcell{\num{1}} & \hebcell{פריט ראשון עם \en{English}} & \hebcell{\percent{10}} & \encell{Active} \\
\hline
\hebcell{\num{2}} & \hebcell{פריט שני} & \hebcell{\num{250.5}} & \encell{Pending} \\
\hline
\hebcell{\num{3}} & \hebcell{פריט שלישי} & \hebcell{\hebyear{2025}} & \encell{Complete} \\
\hline
\hebcell{\num{4}} & \hebcell{פריט רביעי} & \hebcell{\Rsquared = \num{0.95}} & \encell{Active} \\
\hline
\hebcell{\num{5}} & \hebcell{פריט חמישי} & \hebcell{\percent{99.9}} & \encell{Testing} \\
\hline
\end{longtable}
