...
\begin{enumerate}
    ...
    \item \textbf{מיקום נכון:} השתמשו ב-\texttt{[H]} לכפיית מיקום מדויק (ראו סעיף \ref{subsec:positioning})
...
\hebrewsubsection{פתרון בעיות מיקום איורים: \entoc{Float Positioning Solution}}
\label{subsec:positioning}

\textbf{הבעיה:} ב-\en{LaTeX}, האפשרויות \texttt{[h]}, \texttt{[t]}, \texttt{[b]}, \texttt{[p]} הן \emph{הצעות} בלבד.
\en{LaTeX} עלול להתעלם מהן ולהזיז איורים לעמודים מאוחרים יותר, במיוחד כאשר האיורים גדולים.

\textbf{הפתרון:} שימוש בחבילת \en{float} ובאפשרות \texttt{[H]} (אות גדולה):

\begin{enumerate}
    \item \textbf{הוספת החבילה:} הוסיפו בתחילת המסמך:
\begin{english}
\begin{verbatim}
\usepackage{float}
\end{verbatim}
\end{english}
    ...
    \item \textbf{שימוש ב-\en{[H]}:} החליפו את \texttt{[h]} ב-\texttt{[H]}:
\begin{english}
\begin{verbatim}
\begin{figure}[H]
\centering
...
\end{english}
    ...
    \item \textbf{עבור \en{hebrewfigure}:} השתמשו ב-\texttt{[H]} גם כן:
\begin{english}
\begin{verbatim}
\hebrewfigure[H]{...}{caption}
\end{verbatim}
\end{english}
\end{enumerate}

\textbf{השוואת אפשרויות מיקום:}
\begin{itemize}
    \item \texttt{[h]} -- כאן (הצעה, \en{LaTeX} עלול להתעלם)
    \item \texttt{[t]} -- בראש העמוד (הצעה)
    \item \texttt{[b]} -- בתחתית העמוד (הצעה)
    \item \texttt{[p]} -- בעמוד נפרד (הצעה)
    \item \texttt{[H]} -- \textbf{כאן בדיוק} (כפייה, דורש חבילת \en{float})
\end{itemize}
...
\begin{itemize}
    \item \textbf{\en{PDF}:} מומלץ לגרפיקה וקטורית
    \item \textbf{\en{PNG}:} מומלץ לצילומי מסך ודיאגרמות
    \item \textbf{\en{JPG}:} מומלץ לתמונות צילום
    \item \textbf{\en{TikZ/Python}:} ליצירת גרפיקה ב-\en{LaTeX} או \en{Matplotlib}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{פתרון בעיות מיקום:} שימוש ב-\texttt{[H]} עם חבילת \en{float}
