...
\englishauthor{Dr. Yoram Segal}% v7.3.6
\hebrewversion{גרסה \en{\clsversion}}
\coverdisclaimer{מסמך הדגמה לכל פקודות התבנית}% v7.3.6
...
% ==================== v7.3.4 TEST: \enpath INLINE RTL JUSTIFICATION ====================
% Inline \enpath tokens in a running Hebrew paragraph must justify (not be thrown left).
\hebrewsection{בדיקת \en{enpath} בטקסט רץ \entoc{v7.3.4 enpath inline}}
כל מאגר \en{GitHub} חייב לכלול, לכל הפחות: קובץ \enpath{README.md} (הדוח האקדמי); קובצי התצורה (\enpath{config/}); הנתיב \enpath{config/game-rules.toml} וגם \enpath{Game-P2P-Cop-Chase}; קובץ תוכנית העבודה (\enpath{PLAN}); וקובצי המשימות (\enpath{TODO}). קבצים אלה מספרים את סיפור הפיתוח ומאפשרים לבוחן לשחזר את דרך העבודה — כל השורות אמורות להיות מיושרות לשני הצדדים.

% ==================== v7.3.3 TEST: COLORED-BOX INNER PADDING + IN-BOX LISTS ====================
% Boxes with itemize/enumerate must show a clear margin between list content and the frame.
\hebrewsection{בדיקת ריפוד תיבות צבעוניות ורשימות בתוכן \entoc{v7.3.3 box padding}}

\begin{exercisebox}[תרגיל עם רשימה ממוספרת — בדיקת ריפוד]
הטקסט הבא נמצא בתוך תיבה צבעונית עם רשימה ממוספרת; יש לוודא מרווח ברור בין תוכן הרשימה למסגרת התיבה:
\begin{enumerate}
  \item הפריט הראשון ברשימה, עם טקסט ארוך מספיק כדי לבדוק את הגלישה אל מסגרת התיבה משני הצדדים.
  \item הפריט השני, הכולל מונח באנגלית \en{Reinforcement Learning} ומספר \num{12345}.
  \item הפריט השלישי והאחרון.
\end{enumerate}
\end{exercisebox}

\begin{notebox}[הערה עם רשימת תבליטים]
\begin{itemize}
  \item תבליט ראשון בתוך תיבת הערה, לבדיקת הריפוד.
  \item תבליט שני, לבדיקת המרווח השמאלי והימני של התוכן בתוך המסגרת.
\end{itemize}
\end{notebox}

\begin{warningbox}[אזהרה עם מספור]
\begin{enumerate}
  \item שורת אזהרה ממוספרת ראשונה.
  \item שורת אזהרה ממוספרת שנייה.
\end{enumerate}
\end{warningbox}

% ==================== v7.3.6 NEW COMMANDS ====================
\hebrewsection{פקודות חדשות (\en{v7.3.6})}

% Command: \hebtextmath - Hebrew text inside math mode
נוסחה עם טקסט עברי במתמטיקה: $E_{\hebtextmath{אנרגיה}} = mc^2$.

% Environment: ltrblock - centered LTR block
\begin{ltrblock}
This is a centered LTR block produced by the \texttt{ltrblock} environment.
\end{ltrblock}

הפקודות \en{\texttt{\textbackslash englishauthor}} ו-\en{\texttt{\textbackslash coverdisclaimer}}
מודגמות בעמוד השער.

% ==================== COVERAGE DEMOS (remaining public API) ====================
\hebrewsectionnotoc{מקטע ללא תוכן עניינים (\en{\textbackslash hebrewsectionnotoc})}
מקטע זה אינו מופיע בתוכן העניינים. ערך \Rtwo\ מוצג כאן.

\englishsectionunnumbered{Unnumbered English Section}
This section uses \texttt{\textbackslash englishsectionunnumbered}.

\hebrewsection{פריטי רשימה והדגמות סביבה}
\begin{itemize}
  \Hitem{פריט רשימה עברי באמצעות \en{\textbackslash Hitem}}
\end{itemize}
...
\begin{exercise}
תרגיל לדוגמה בסביבת \en{\texttt{exercise}}.
\end{exercise}

\begin{codebox}
דוגמת תיבת קוד (\en{\texttt{codebox}}).
\end{codebox}

\begin{latin}
This text is inside the \texttt{latin} environment (English alias).
\end{latin}

זכויות יוצרים בכותרת התחתונה: \hebfoot{כל הזכויות שמורות - \en{\textcopyright\ Segal}}.

% ==================== v7.4.2 TEST: TOC SUBSECTION CHAPTER PREFIX ====================
% The TOC entry for a \hebrewsubsection must carry the FULL compound number
% "chapter.section.subsection", not "section.subsection".
% Before v7.4.2 the chapter component was dropped in the .toc only (the printed
% body heading was already correct), so a subsection of section 2.N was listed as
% "N.1" and collided with a real section of chapter N.
%
% VERIFY IN THE PDF (table of contents, not just this page):
%   - the three entries below appear as 2.X.1 / 2.X.2 / 2.X.3 where X is this
%     section's number within chapter 2 -- three components, chapter first
%   - each renders left-to-right (NOT reversed, e.g. never "1.X.2")
%   - the body headings on this page show the SAME numbers as the TOC entries
\hebrewsection{בדיקת מספור תת-סעיפים בתוכן העניינים \entoc{v7.4.2 TOC subsection prefix}}
...
\hebrewsubsection{תת-סעיף שלישי: \entoc{Third Subsection}}
תת-סעיף שלישי, לאימות שהמונה מתקדם כראוי ואינו מתאפס.

% ==================== v7.4.3 TEST: API SYMMETRY ====================
% Two commands added in v7.4.3 to close asymmetries in the public API.
%
% VERIFY IN THE PDF:
%   1. \starthebrew  - the Hebrew paragraph below it is RTL and ragged-left, exactly
%      like the paragraph after \stophebrew. Before v7.4.3 this line raised
%      "Undefined control sequence" even though the docs listed the command.
%   2. \englishsubsection - renders an LTR English subsection heading whose number has
%      THREE components (chapter.section.subsection), and whose TOC entry matches the
%      heading. It must sit in the same number sequence as the \hebrewsubsection
%      entries above it -- i.e. it continues the count, it does not restart it.
\hebrewsection{בדיקת סימטריית הפקודות \entoc{v7.4.3 API symmetry}}

הפקודה \en{\texttt{\textbackslash startenglish}} מתחילה גוש אנגלי, והפקודה החדשה
\en{\texttt{\textbackslash starthebrew}} מתחילה גוש עברי -- כך ששני הצדדים סימטריים.

\startenglish
This paragraph was opened with \texttt{startenglish}: it is left-to-right and
ragged-right, and it ends where the Hebrew block begins.
\stopenglish

\starthebrew
פסקה זו נפתחה בפקודה החדשה \en{\texttt{\textbackslash starthebrew}}. עליה להופיע מימין
לשמאל ומיושרת לשמאל, בדיוק כמו פסקה שנפתחה ב-\en{\texttt{\textbackslash stophebrew}}.
\stophebrew

\hebrewsubsection{תת-סעיף עברי לפני האנגלי: \entoc{Hebrew Subsection Before}}
תת-סעיף עברי רגיל, כדי שניתן יהיה לוודא שתת-הסעיף האנגלי שאחריו ממשיך את אותו מספור.

\englishsubsection{English Subsection (v7.4.3)}
This subsection heading was produced by \texttt{englishsubsection}. Its number must have
three components and must continue the sequence started by the Hebrew subsection above.

\hebrewsubsection{תת-סעיף עברי אחרי האנגלי: \entoc{Hebrew Subsection After}}
