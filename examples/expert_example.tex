...
# הדגמת קוד Python עם הדגשת תחביר
def fibonacci(n):
...
# בלוק קוד לא-צף עבור רישומים ארוכים
class DataProcessor:
...
class MultiHeadAttention(nn.Module):
    """
    מנגנון attention רב-ראשי
    משמש בארכיטקטורת Transformer
    """
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # טרנספוזיציה עבור attention
        Q = Q.transpose(1, 2)
        K = K.transpose(1, 2)
        V = V.transpose(1, 2)

        # attention עם מכפלה סקלרית מנורמלת
